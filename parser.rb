...
class Parser
	...
	def check_token_hanging_indent(begin_pos, token)
		warns = []
		if begin_pos.line != token.pos.line
			@indent_state = @indent_state.set_hanging(true)
			warns += check_token_indent(token)
		end
		return warns
	end
	...
	def parse_multiple_variable_declaration_we(var_decl_token)
		...
		while true
			...
			if comma_token.is_a?(CommaToken)
				...
				ws3 += check_token_hanging_indent(var_decl_token_begin_pos, var_decl_token)
				warns += ws3
	...
	def parse_paren_variable_declaration_we(left_paren_token)
		...
		ws1 += check_token_hanging_indent(mvar_decl_token_begin_pos, mvar_decl_token)
		warns += ws1
	...
	def parse_assignment_expression_we(exp_token)
		...
		indent = nest_indent()
		ws2 += check_token_hanging_indent(equal_token_begin_pos, equal_token)
		warns += ws2
